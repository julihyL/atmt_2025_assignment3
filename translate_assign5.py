...
def get_args():
    ...
    parser.add_argument('--beam-size', default='5', type=str, help='comma-separated list of beam sizes for beam search decoding')
    ...
    args.beam_size = parse_beam_sizes(args.beam_size)
    return args
...
def main(args):
    ...
    for name, funcs in FUNC.items():
        ...
        for beam_size in args.beam_size:
            ...
            for batch in tqdm(batch_iter(src_encoded, args.batch_size), desc=f"Beam size {beam_size}"):
                with torch.no_grad():
                    ...
                    if beam_size == 1:
                        prediction = funcs[1](model=model,
                        ...
                        prediction = funcs[0](model=model,
                                                    src_tokens=src_tokens,
                                                    src_pad_mask=src_pad_mask,
                                                    max_out_len=args.max_len,
                                                    tgt_tokenizer=tgt_tokenizer,
