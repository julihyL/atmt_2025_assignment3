...
def decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
    ...
    # Cache previous decoder states
    prev_states = None
    for t in range(max_out_len):
        last_token = generated[:, -1:].clone()
        trg_pad_mask = (last_token == PAD).unsqueeze(1).unsqueeze(2)
        output, prev_states = model.step(src_tokens, src_pad_mask, last_token, trg_pad_mask, prev_states)
        next_tokens = output[:, -1, :].argmax(dim=-1, keepdim=True)
...
def decode_opt(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
           tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device):
    """Decodes a sequence without teacher forcing. Works by relying on the model's own predictions, rather than the ground truth (trg_)"""
    batch_size = src_tokens.size(0)
    BOS = tgt_tokenizer.bos_id()
    ...
    for _ in range(max_out_len):
        ...
        for seq, score in beams:
            ...
            with torch.no_grad():
                ...
                trg_pad_mask = (seq[:, -1:] == PAD)[:, None, None, :]
                output, prev_states = model.step(src_tokens, src_pad_mask, seq[:, -1:], trg_pad_mask, None)
                logits = output[:, -1, :]
...
def beam_search_decode_opt(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                       tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7):
    """Beam Search decoding compatible with Transformer-based Seq2Seq models."""
    model.eval()
    BOS, EOS, PAD = tgt_tokenizer.bos_id(), tgt_tokenizer.eos_id(), tgt_tokenizer.pad_id()
